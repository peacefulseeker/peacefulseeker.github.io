...
test.describe("One-page fill gauge", () => {
  for (const theme of THEME_NAMES) {
    test(`the one-pager fits a single A4 page in print — ${theme} theme`, async ({
      page,
    }) => {
      ...
      expect(
        pages,
        `one-pager (${theme} theme) spilled past a single A4 page — trim it via onepage_highlights_num / onepage_include / summary_short in src/content/resumes/alexey-vorobyov.md, or adjust the ${theme} onepage density tokens in src/styles/resume.css`,
      ).toBe(1);
    });
  }
});
