...
## Account check (run first)

Before doing anything else in this project, verify the correct Claude Code
config directory is active:

1. Run: `echo "$CLAUDE_CONFIG_DIR"`
2. Expected value: `<absolute path to ~/.claude-zartis>` (resolve `~` —
   the comparison must be against the absolute path)
3. If the value is empty, or does not match, STOP. Do not run any further
   tools, do not create or modify files. Tell the user:
   "CLAUDE_CONFIG_DIR is not set to the Zartis account
   (got: <actual>, expected: <expected>). Exit Claude Code, set the
   variable (or `cd` into the project so mise activates it), and start
   a new session."

## Claude Account Settings

Always use the `$CLAUDE_CONFIG_DIR` environment variable to reference Claude account settings, sessions, and configuration files. Never hardcode absolute paths like `~/.claude` or `/Users/<username>/.claude`.

Examples:

- Settings: `$CLAUDE_CONFIG_DIR/settings.json`
- MCP config: `$CLAUDE_CONFIG_DIR/settings.json` (mcpServers section)
